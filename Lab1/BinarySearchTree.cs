...
namespace Lab1Tree
{
    public class BinarySearchTree<T> : IEnumerable<T>
    {
        ...
        public IEnumerable<T> BypassTree(BypassType bypassType)
        {
            switch (bypassType)
            {
                case BypassType.PreOrder:
                    return PreOrderBypassTree(m_root);
                case BypassType.InOrder:
                    return InOrderBypassTree(m_root);
                case BypassType.PostOrder:
                    return PostOrderBypassTree(m_root);
                default:
        ...
        {
            var parentCompation = m_comparer.Compare(item, parent.Item);
